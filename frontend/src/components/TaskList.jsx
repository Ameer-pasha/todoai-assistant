...
export default function TaskList({
  ...
  const filtered = useMemo(() => {
    ...
    switch (activeFilter) {
      ...
      case 'work':
      case 'personal':
      case 'urgent': return tasks.filter((t) => taskHasTag(t, activeFilter));
  ...
  const canAddTaskHere = activeFilter === 'all';
      ...
      <div className="task-list-scroll">
        {sorted.length === 0 && (
          <div className="task-empty fade-in">
            ...
            <h3>No tasks here</h3>
            <p>{canAddTaskHere ? 'Use Add Task to create one manually.' : 'Switch to All Tasks to add a new task.'}</p>
          </div>
        )}
